<?php
	...
	$stmt2->execute(array("user_ID" => $_SESSION['manage_uid']));
	$row2 = $stmt2->fetch();
	$total_space = (((int)$row2[0]/1024)/1024);
	echo "總空間：".$total_space."MB <BR>";
	...
	$stmt3->execute(array("user_ID" => $_SESSION['manage_uid']));
	$row3 = $stmt3->fetch();
	//$userd_space = ceil(((int)$row3[0]/1024)/1024)-$total_space;
	$userd_space = ceil(((int)$row3[0]/1024)/1024);
	/*
	echo "&nbsp;&nbsp;&nbsp;&nbsp;子帳號配置空間：".$userd_space."MB <BR>";
	
	if ($userd_space > $total_space){
		echo "&nbsp;&nbsp;&nbsp;&nbsp;子帳號可配置空間：<font color=\"red\"> 0 </font>MB <BR>";
	} else {
		echo "&nbsp;&nbsp;&nbsp;&nbsp;子帳號可配置空間：".($total_space-$userd_space)."MB <BR>";
	}
	*/
